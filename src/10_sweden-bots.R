...
# Wikipedia Swedish Edition (svwiki)
# =============================================================================
...
cat("Rows loaded:", nrow(sve_data),
    "| Bot share:", round(mean(sve_data$is_bot) * 100, 1), "%\n")
...
# FIX: was eng_data — now correctly uses sve_data
features_df <- sve_data$edit_types_json %>%
  ...
  mutate(
    ...
    is_bot    = sve_data$is_bot[row_id],      # FIX
    page_id   = sve_data$page_id[row_id],     # FIX
    user_text = sve_data$user_text[row_id]    # FIX
  )
...
mean_by_bot %>%
  ...
  labs(title = "Edit type features: bot vs. human (log2 ratio) — svwiki",
...
# 7. COMMENT FEATURES
# -----------------------------------------------------------------------------
# NOTE: Swedish bots may self-identify as "robot" (Swedish for "bot/robot")
# in addition to the English "bot". The patterns below catch both.

# FIX: was eng_data — now correctly uses sve_data
comment_features <- sve_data %>%
  mutate(
    ...
    mentions_bot        = str_detect(tolower(revision_comment), "bot|robot"),  # "robot" is Swedish for bot
    mentions_automated  = str_detect(tolower(revision_comment),
                                     "automated|script|auto|automatisk"),      # + Swedish "automatisk"
    ...
    mentions_fix        = str_detect(tolower(revision_comment),
                                     "fix|correct|clean|r\u00e4tta|korrigera"), # + "rätta", "korrigera"
    mentions_revert     = str_detect(tolower(revision_comment),
                                     "revert|undo|rv\\b|\u00e5terst\u00e4ll|rullar"),  # + "återställ", "rullar"
...
# FIX: was eng_data$is_bot — now correctly uses sve_data$is_bot
comment_features %>%
  bind_cols(is_bot = sve_data$is_bot) %>%
  group_by(is_bot) %>%
...
# FIX: match against sve_data row count (not eng_data)
features_combined <- bind_cols(
  features_model,
  comment_features_clean[match(features_model$row_id, seq_len(nrow(sve_data))), ]  # FIX
)
